...
param           : IDENT TYPEOF type_spec                        { Debug("param -> IDENT :: type_spec"           ); $$ = param____IDENT_TYPEOF_typespec($1, $2, $3); }
                ;
...
prim_type       : NUM                                           { Debug("prim_type -> NUM"                      ); $$ = primtype____NUM($1); }
                | BOOL                                          { Debug("primtype -> BOOL"                      ); $$ = primtype____BOOL($1); }
                ;
...
print_stmt      : PRINT expr SEMI                               { Debug("print_stmt -> print expr ;"            ); $$ = print_stmt____PRINT_expr_SEMI($1,$2,$3); }
                ;
...
while_stmt      : WHILE expr BEGIN stmt_list END                { Debug("while_stmt -> WHILE expr BEGIN stmt_list END"); $$ = while_stmt____WHILE_expr_BEGIN_stmt_list_END($1,$2,$3,$4,$5); }
                ;
...
arg_list        : arg_list COMMA expr                           { Debug("arg_list -> arg_list , expr"           ); $$ = arg_list____arg_list_COMMA_expr($1,$2,$3); }
...
expr            : expr ADD expr                                 { Debug("expr -> expr ADD expr"                 ); $$ = expr____expr_ADD_expr           ($1,$2,$3   ); }
                ...
                | IDENT LPAREN args RPAREN                      { Debug("expr -> IDENT ( args )"                ); $$ = expr____IDENT_LPAREN_args_RPAREN($1,$2,$3,$4); }
